...
import { BANNER_TYPE, IMAGE_TYPE } from "../../utils/app-enumeration";
import {
  BANNER_NOT_FOUND,
  ...
  try {
    ...
    if (req.body.only_active_inactive === "1") {
      await Banner.update(
        ...
        { where: { id: bannerToUpdate.dataValues.id } }
      );
...
export const statusUpdateBanner = async (req: Request) => {
  try {
      const BannerExists = await Banner.findOne({ where: { id: req.body.id,  banner_type: { [Op.eq]: BANNER_TYPE.banner }, is_deleted: "0" } });
      if (BannerExists) {
