  ...
  try {
    ...
    const pass_hash = await bcrypt.hash(password, Number(PASSWORD_SOLT));
      ...
      try {
        ...
        const emailExistes = await customerUser.findOne({ where: { email: email, is_deleted: "0" } })
        const emailidExistes = await AppUser.findOne({ where: { username: email, is_deleted: "0" } })
...
try {
  ...
  try {
    ...
    const customerEmailExistes = await customerUser.findOne({ where: { email: email,id: { [Op.ne]: id }, is_deleted: "0" } })
    const appUserEmailidExistes = await AppUser.findOne({ where: { username: email,id: {[Op.ne]: CustomerId.dataValues.id_app_user}, is_deleted: "0" } })
