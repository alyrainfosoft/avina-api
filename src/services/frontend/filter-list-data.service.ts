...
export const diamondFilterListAPI =async (req: Request) => {
    try {
      ...
      const caratWeightData = await CaratSize.findAll({
        where,
        order: [
            ['value', 'ASC'],
        ],
    ...
    return resSuccess({ data: {caratWeight: caratWeightData, colorData, clarityData, minPrice: Math.min(...maxPrice), maxPrice: Math.max(...maxPrice) } })
...
export const metalFilterListAPI =async (req: Request) => { 
  ...
  try {
    ...
    const settingStyleData = await SettingType.findAll({
      where,
      attributes:[
        "id",
        "name",
    ...
    return resSuccess({data:{ diamondShapeData, settingStyleData, metalToneData}})
