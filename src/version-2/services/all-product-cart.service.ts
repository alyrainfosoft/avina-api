...
export const addAllTypeProductWithPaypalOrder = async (req: Request) => {
  try {
    ...
    let allTax = [];
    let taxRateData = [];
    const sub_total_value: any = parseFloat(sub_total);
    for (const taxData of taxValues) {
      productTax = taxData.dataValues.rate / 100;
      productTaxAmount = sub_total_value.toFixed(2) * productTax;
    ...
    try {
      ...
      if (order_shipping_address && shipping_method == SHIPPING_METHOD.online) {
      ...
      for (let product of product_details) {
        ...
        if (AllProductTypes.Product == product.product_type) {
          ...
          const metalRates: any = await dbContext.query(
            ...
            ELSE (metal_master.metal_rate/31.104*gold_kts.name/24*birthstone_PMO.metal_weight) 
            ...
            AND birthstone_products.is_deleted = '0'
            END GROUP BY metal_master.metal_rate, birthstone_PMO.metal_weight, birthstone_products.making_charge, birthstone_products.finding_charge, birthstone_products.other_charge,birthstone_PMO.id_karat, gold_kts.name`,
            { type: QueryTypes.SELECT }
          );
...
WHERE
	CPDO.ID = ${product.product_id}`,
      ...
      if (payment_method == paymentMethod.paypal) {
        ...
        const options = {
          amount: parseFloat(order_total) * 100,
