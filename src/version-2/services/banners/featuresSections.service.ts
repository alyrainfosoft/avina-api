...
export const addFeaturesSections = async (req: Request) => {
  const { name, expiry_date, created_by, active_date, content } = req.body
  try {
    let imagePath = null;
    if (req.file) {
    ...
    try {
      ...
      const FeaturesSections = await Banner.create(
        {
          name: name,
...
export const updateFeaturesSections = async (req: Request) => {
  const { id, name, content, active_date, updated_by, expiry_date } = req.body

  try {

    const bannerId = await Banner.findOne({ where: { id: id, banner_type: { [Op.eq]: BANNER_TYPE.features_sections }, is_deleted: "0" } })
...
export const deleteFeatureSection = async (req: Request) => {
  ...
  try {
    const FeatureSectionExists = await Banner.findOne({ where: { id: req.body.id, banner_type: { [Op.eq]: BANNER_TYPE.features_sections }, is_deleted: "0" } });
