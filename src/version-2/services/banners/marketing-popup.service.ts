...
export const updateMarketingPopup = async (req: Request) => {
  ...
  try {
      ...
      const popupId = await Banner.findOne({ where: { id: id, banner_type: { [Op.eq]: BANNER_TYPE.marketing_popup }, is_deleted: "0" } })
    if (popupId == null) {
      return resNotFound() 
    }
    ...
    try {
      ...
      if (id_image === null) {
        const popupInfo = await (Banner.update(
          ...
          const popupInformation = await Banner.findOne({ where: { id: id, is_deleted: "0" }, transaction: trn })
...
export const deleteMarketingPopup = async (req: Request) => {
    ...
    try {
        const marketingPopupExists = await Banner.findOne({ where: { id: req.body.id, banner_type: { [Op.eq]: BANNER_TYPE.marketing_popup }, is_deleted: "0" } });

        console.log(marketingPopupExists)
