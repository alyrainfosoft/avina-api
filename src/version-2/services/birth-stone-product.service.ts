...
export const birthstoneProductPriceFind = async (req: Request) => {
  try {
    ...
    ELSE (metal_master.metal_rate/31.104*gold_kts.name/24*birthstone_PMO.metal_weight+(COALESCE(making_charge, 0))+(COALESCE(finding_charge, 0))+(COALESCE(other_charge, 0))+(COALESCE(sum(DGM.rate*birthstone_PDO.count), 0))) 
