...
const addProductToDB = async (productlList: any, idAppUser: number) => {
  ...
  try {
    ...
    await BirthstoneProductCategory.bulkCreate(pcPayload, { transaction: trn });
    await BirthstoneProductMetalOption.bulkCreate(pmoPayload, {
      transaction: trn,
    });
    await BirthStoneProductDiamondOption.bulkCreate(pdoPayload, {
      transaction: trn,
    });
    await BirthstoneProductEngraving.bulkCreate(engravingPayload, {
      transaction: trn,
    });
