...
import { getInitialPaginationFromQuery, getLocalDate, resErrorDataExit, resNotFound, resSuccess, resUnknownError } from "../../utils/shared-functions"
import { DEFAULT_STATUS_CODE_SUCCESS, IMAGES_NOT_FOUND, PRODUCT_EXIST_WITH_SAME_SKU, PRODUCT_NOT_FOUND, RECORD_DELETE_SUCCESSFULLY, RECORD_UPDATE_SUCCESSFULLY, THUMB_IMAGE_ONLY_ONE, VIDEO_ONLY_ONE } from "../../utils/app-messages"
import { ActiveStatus, AllProductTypes, GIFT_PRODUCT_IMAGE_TYPE } from "../../utils/app-enumeration"
import dbContext from "../../config/db-context"
import { moveFileToS3ByTypeAndLocation } from "../../helpers/file.helper"
import { PRODUCT_FILE_LOCATION, PRODUCT_PER_PAGE_ROW } from "../../utils/app-constants"
...
export const addGiftSetProductAPI = async (req: Request) => {
    ...
    try {
        ...
        const {sku, product_title, short_description, brand_id,  long_description, tags, gender, price, thumb_images, featured_images } = req.body

        let slug = product_title.replaceAll(" ", "-") 
        const productsku = await GiftSetProduct.findOne({ where: { sku: sku, is_deleted: "0" } })
        ...
        try {
            ...
            const configProduct = await GiftSetProduct.create(payload, {transaction: trn})
            let videoFile;
    ...
    try {
      ...
      let where = [
        ...
        pagination.search_text
          ? {
            [Op.or]: [
              { product_title: { [Op.iLike]: "%" + pagination.search_text  + "%" } },
...
export const editGiftSetProductApi =async (req:Request) => {
    try {
        ...
        const {id, sku, product_title, brand_id, short_description, long_description, tags, gender, price} = req.body

        let slug = product_title.replaceAll(" ", "-") 
        const products = await GiftSetProduct.findOne({ where: { id: id, is_deleted: "0" } })

        if(!(products && products.dataValues)) {
            return resNotFound({message: PRODUCT_NOT_FOUND})
        }

        const productsku = await GiftSetProduct.findOne({ where: { sku: sku, id: {[Op.ne]: id}, is_deleted: "0" } })
        ...
        try {
            ...
            const configProduct = await GiftSetProduct.update(
                payload, 
            ...
            if(files['video']) {
              ...
              for(let imageList of vedioFind) {
                ...
                await (GiftSetProductImages.destroy(
                  { where: { id: imageList.dataValues.id, id_product: imageList.dataValues.id_product } }
              ));
...
export const statusUpdateGiftSetProduct = async (req: Request) => {
    try {
        const GiftSetProductExists = await GiftSetProduct.findOne({ where: { id: req.body.id, is_deleted: "0" } });
...
export const deleteGiftSetProductImage = async (req: Request) => {
    try {
        ...
        const product = await GiftSetProduct.findOne({ where: { id: id_product, is_deleted: "0" } });

        if(!(product && product.dataValues)) {
          return resNotFound({message: PRODUCT_NOT_FOUND})
        }

        const productImage = await GiftSetProductImages.findOne({ where: { id: id, is_deleted: "0" } });
...
export const getAllGiftSetProductsUserSide = async (req: Request) => {
  try {
    ...
    let where = [
      ...
      {is_active: ActiveStatus.Active},
      req.query.brand ?
      Sequelize.where(Sequelize.literal(`(SELECT COUNT(*) FROM brands WHERE brands.id = gift_set_products.brand_id AND brands.name ILIKE  '%${req.query.brand}%')`) , ">", "0") 
      : {} ,
      pagination.search_text
      ? {
        [Op.or]: [
    ...
    const result = await GiftSetProduct.findAll({
      ...
      attributes: [
        ...
        "long_des",
        "brand_id",
        [Sequelize.literal(`(SELECT id FROM wishlist_products WHERE product_id = "gift_set_products"."id" AND product_type = ${AllProductTypes.GiftSet_product}  AND user_id = ${req.query.user_id && req.query.user_id != '' ? req.query.user_id: 0})`), "wishlist_id"],
        [
          Sequelize.literal(
            `CASE WHEN "gift_set_products"."tags" IS NULL THEN '{}'::int[] ELSE string_to_array("gift_set_products"."tags", '|')::int[] END`
          ),
  ...
  try {
    ...
    const result = await GiftSetProduct.findOne({
      ...
      attributes: [
        ...
        "long_des",
        "brand_id",
        [Sequelize.literal(`(SELECT id FROM wishlist_products WHERE product_id = "gift_set_products"."id" AND product_type = ${AllProductTypes.GiftSet_product}  AND user_id = ${req.body.user_id && req.body.user_id != '' ? req.body.user_id: 0})`), "wishlist_id"],
        [
          Sequelize.literal(
            `CASE WHEN "gift_set_products"."tags" IS NULL THEN '{}'::int[] ELSE string_to_array("gift_set_products"."tags", '|')::int[] END`
          ),
