...
import Masters from "../../model/master/master.model";
import {
  ActiveStatus,
  DeletedStatus,
  Image_type,
...
export const addMaster = async (req: Request) => {
  try {
    ...
    const duplicateName = await Masters.findOne({
      where: [
    ...
    const duplicateSortCode = await Masters.findOne({
      where: [
    ...
    if (id_parent) {
      const parentIdCheck = await Masters.findOne({
        where: { id: id_parent },
      });
      if (!parentIdCheck) {
        return resBadRequest({
...
export const masterList = async (req: Request) => {
  try {
    ...
    const totalItems = await Masters.count({
      where,
    });
    ...
    const Master = await Masters.findAll({
      where,
    ...
    return resSuccess({ data: { pagination, result: Master } });
  } catch (error) {
    throw error;
  }
};
...
export const masterDelete = async (req: Request) => {
  try {
    ...
    const ParentData = await Masters.findAll({
      where: {
        id_parent: id,
        master_type: master_type,
        is_deleted: DeletedStatus.No,
