...
import { ActiveStatus, BANNER_TYPE, IMAGE_TYPE, TEMPLATE_2_BANNER_TYPE } from "../../../utils/app-enumeration";
import {
  BANNER_NOT_FOUND,
...
export const updateTemplateTwoHomeAboutBanner = async (req: Request) => {
  try {
    const bannerToUpdate = await TemplateTwoBanner.findOne({
      where: { id: req.body.id, is_deleted: '0', banner_type: TEMPLATE_2_BANNER_TYPE.home_about_banner },
    });
...
export const statusUpdateTemapleTwoHomeAboutBanner = async (req: Request) => {
  try {
      const BannerExists = await TemplateTwoBanner.findOne({ where: { id: req.body.id,  banner_type: { [Op.eq]: TEMPLATE_2_BANNER_TYPE.home_about_banner }, is_deleted: "0" } });
      if (BannerExists) {
