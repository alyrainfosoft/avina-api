...
export const addTemplateTwoHomeAboutFeaturesSections = async (req: Request) => {
  const { title, target_url, button_name, content, sub_title } = req.body
  try {
    let imagePath = null;
    if (req.file) {
...
export const updateTemplateTwoHomeAboutFeaturesSections = async (req: Request) => {
  const { id, title, content, target_url,  button_name, sub_title } = req.body

  try {

    const bannerId = await TemplateTwoBanner.findOne({ where: { id: id, banner_type: { [Op.eq]: TEMPLATE_2_BANNER_TYPE.home_about_features_section }, is_deleted: "0" } })
...
export const deleteTemplateTwoHomeAboutFeatureSection = async (req: Request) => {
  ...
  try {
    const FeatureSectionExists = await TemplateTwoBanner.findOne({ where: { id: req.body.id, banner_type: { [Op.eq]: TEMPLATE_2_BANNER_TYPE.home_about_features_section }, is_deleted: "0" } });
