...
export const addTemplateTwoHomeAboutMarketingSection = async (req: Request) => {
  try {
    let imagePath = null;
    if (req.file) {
...
export const updateTemplateTwoHomeAboutMarketingSection = async (req: Request) => {
  try {
    const bannerToUpdate = await TemplateTwoBanner.findOne({
      where: { id: req.body.id, is_deleted: '0', banner_type: TEMPLATE_2_BANNER_TYPE.home_about_marketing_section },
    });
...
export const statusUpdateTemapleTwoHomeAboutMarketingSection = async (req: Request) => {
  try {
      const BannerExists = await TemplateTwoBanner.findOne({ where: { id: req.body.id,  banner_type: { [Op.eq]: TEMPLATE_2_BANNER_TYPE.home_about_marketing_section }, is_deleted: "0" } });
      if (BannerExists) {
