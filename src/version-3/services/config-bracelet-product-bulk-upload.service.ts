...
`;
    ...
    await ConfigBraceletProductDiamonds.bulkCreate(productDiamondPayload, {
      transaction: trn,
    });
    await ConfigBraceletProductMetals.bulkCreate(prodMetalPayload, {
      transaction: trn,
    });

    await trn.commit();
    await refreshMaterializedBraceletConfiguratorPriceFindView(dbContext);
    return resSuccess();
  } catch (e) {
    await trn.rollback();
    return resUnknownError({ data: e });
  ...
  try {
    ...
    try {
      ...
      await ConfigBraceletProductDiamonds.update(
        { is_deleted: DeletedStatus.yes },
        { where: { config_product_id: req.params.id } }
      );
...
export const braceletConfiguratorProductPriceFind = async (req: Request) => {
  try {
    const {
      product_style,
      product_length,
    ...
    const result = await dbContext.query(
      ...
      metal_weight_type,
      total_diamond_wt,
      CEIL(product_price) as product_price,
      metals,
      diamond_details
