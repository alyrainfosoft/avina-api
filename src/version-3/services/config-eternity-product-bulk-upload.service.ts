...
const addProductToDB = async (productList: any, idAppUser: number) => {
  ...
  try {
    ...
    await trn.commit();
    await refreshMaterializedEternityBandConfiguratorPriceFindView(dbContext);
    return resSuccess();
  } catch (e) {
    await trn.rollback();
    return resUnknownError({ data: e });
  ...
  try {
    ...
    try {
      await ConfigEternityProduct.update(
        ...
        { where: { id: req.params.product_id }, transaction: trn }
      );
...
export const eternityPriceFind = async (req: Request) => {
  try {
    ...
    let product = await dbContext.query(
      `(SELECT CEBP.id,side_setting_id,product_title,product_sort_des,product_long_des,
          ...
          CEIL(calculated_value) as product_price,CEBP.metal_weight,CEBP.diamond_weight FROM eternity_band_configurator_price_view as CEBP
      ...
      } and CEBP.id_stone = ${stone} 
          AND CEBP.dia_type = ${diamond_type}
          AND product_size = '${ringSize}' AND metal_id = ${metal} AND karat_id = ${karat}
          ${
            gemstone &&
            gemstone.stone &&
            gemstone.cut &&
            ...
            gemstone.cut != ""
              ? gemstone.stone == stone && gemstone.cut == cut
                ...
                : `AND CAST(diamonds ->> 'dia_stone' as integer) = ${gemstone.stone} 
                    AND  CAST(diamonds ->> 'dia_cuts' as integer) = ${gemstone.cut}`
              : cut && cut != null
              ? `AND product_combo_type = 3`
    ...
    if (
      ...
      product = await dbContext.query(
        `(SELECT CEBP.id,side_setting_id,product_title,product_sort_des,product_long_des,
            ...
            WHERE side_setting_id = ${side_setting} 
            AND CEBP.dia_cts = ${caratSize} and CEBP.dia_shape_id = ${shape} and CEBP.dia_cut_id ${
          cut && cut != null ? `= ${gemstone.cut}` : `IS NULL`
        } 
            and CEBP.dia_color ${
              color && color != null ? `= ${color}` : `IS NULL`
            } and CEBP.dia_clarity_id  ${
          clarity && clarity != null ? `= ${clarity}` : `IS NULL`
        } and CEBP.id_stone = ${gemstone.stone} 
            AND CEBP.dia_type = ${diamond_type}
            AND product_size = '${ringSize}' AND metal_id = ${metal} AND karat_id = ${karat}
            ${
              gemstone &&
              ...
              gemstone.cut != ""
                ? gemstone.stone == stone && gemstone.cut == cut
                  ? `AND product_combo_type = 3`
                  : `AND CAST(diamonds ->> 'dia_stone' as integer) = ${stone} 
                      AND  CAST(diamonds ->> 'dia_cuts' as integer) = ${cut}`
                : `AND product_combo_type = 1`
            })
            `,
        { type: QueryTypes.SELECT }
