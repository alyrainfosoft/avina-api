  ...
  try {
    ...
    const result = await ConfigProduct.findAll({
      ...
      attributes: [
        ...
        [
          Sequelize.literal(
            `(SELECT name FROM diamond_shapes WHERE id = ${"cender_diamond.id_shape"})`
          ),
          "center_Diamond_shape",
        ],
        [
          Sequelize.literal(
            `(SELECT name FROM gemstones WHERE id = ${"cender_diamond.id_stone"})`
          ),
          "center_stone",
        ],
        [
          Sequelize.literal(
            `(SELECT value FROM colors WHERE id = ${"cender_diamond.id_color"})`
          ),
          "center_diamond_color",
        ],
        [
          Sequelize.literal(
            `(SELECT value FROM clarities WHERE id = ${"cender_diamond.id_clarity"})`
          ),
          "center_diamond_clarity",
        ],
        [
          Sequelize.literal(
            `(SELECT value FROM carat_sizes WHERE id = ${"cender_diamond.id_carat"})`
          ),
          "center_diamond_carat",
        ],
      ...
      include: [
        ...
        { model: DiamondGroupMaster, as: "cender_diamond", attributes: [] },
      ],
    });
