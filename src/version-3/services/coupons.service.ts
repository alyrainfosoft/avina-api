  ...
  WHERE CONFIG_BRACELET_PRODUCTS.ID = "product_id") ELSE null END)`),
   ...
   WHEN "product_type" = ${
     AllProductTypes.GiftSet_product
      ...
      ELSE (metal_master.metal_rate/metal_master.calculate_rate*gold_kts.calculate_rate*birthstone_PMO.metal_weight+(COALESCE(making_charge, 0))+(COALESCE(finding_charge, 0))+(COALESCE(other_charge, 0))+(COALESCE(sum(DGM.rate*birthstone_PDO.count), 0))+(CAST (product_details ->> 'selected_stone_price' AS DECIMAL(12, 1)))) 
      ...
      END GROUP BY metal_master.metal_rate, metal_master.calculate_rate, birthstone_PMO.metal_weight, birthstone_products.making_charge, birthstone_products.finding_charge, birthstone_products.other_charge,birthstone_PMO.id_karat, gold_kts.calculate_rate) WHEN "product_type" = ${
        AllProductTypes.Eternity_product
      } THEN (SELECT 
      (((CASE 
          WHEN CAST(product_details ->> 'diamond_type' AS integer) = 1 
...
WHERE CONFIG_BRACELET_PRODUCTS.ID = "product_id") ELSE null END`),
    ...
    if (recordsExists.dataValues.min_total_amount > amount) {
      return resBadRequest({
        data: amount,
        message: prepareMessageFromParams(MIN_MAX_REQUIRED_AMOUNT_ERROR, [
          ["min_amount", `${recordsExists.dataValues.min_total_amount}`],
          ["max_amount", `${recordsExists.dataValues.max_total_amount}`],
        ]),
      }); //
    }
    ...
    console.log("-----------------", recordsExists.dataValues);
