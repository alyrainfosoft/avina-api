  ...
  try {
    const diamondGroupExit = await DiamondGroupMaster.findOne({
      where: {
        ...
        id_mm_size: id_mm_size == "null" ? null : id_mm_size,
    ...
    try {
      ...
      await trn.commit();
      await addActivityLogs(req,LOG_FOR_SUPER_ADMIN,[{ old_data: null, new_data: data.dataValues }], data.dataValues.id, LogsActivityType.Add, LogsType.DiamondGroupMater, req.body.session_res.id_app_user)
  ...
  try {
    ...
    const result = await DiamondGroupMaster.findAll({
      ...
      include: [
        ...
        { model: CaratSize, as: "carats", attributes: [] },
        { model: SieveSizeData, as: "seive_size", attributes: [] },
      ],
    });
...
export const updateDiamondGroup = async (req: Request) => {
  ...
  try {
    ...
    const findName = await DiamondGroupMaster.findOne({
      where: {
        id_stone: id_stone,
        id_shape: id_shape,
    ...
    try {
      ...
      if (updateData) {
        ...
        await addActivityLogs(req,LOG_FOR_SUPER_ADMIN,[{ old_data: diamondMasterId.dataValues, new_data: { ...diamondMasterId.dataValues, ...payload } }], diamondMasterId.dataValues.id, LogsActivityType.Edit, LogsType.DiamondGroupMater, req.body.session_res.id_app_user)
...
const getDiamondGroupFromRows = async (rows: any) => {
  ...
  let where = [{ is_deleted: DeletedStatus.No }, { is_active: "1" }];
  ...
  const carat_size = await CaratSize.findAll({
    where,
    attributes: ["id", "value", "slug"],
  });
