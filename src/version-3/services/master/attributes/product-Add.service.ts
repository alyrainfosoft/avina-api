  ...
  try {
    ...
    const categoryList = await categoryData.findAll({
      ...
      include: [
        ...
        { model: categoryData, as: "parent_category", attributes: [] },
      ],
    });
...
export const dashboardAPI = async (req: Request) => {
  try {
    ...
    const total_order = await Orders.count({where: {payment_status: PaymentStatus.paid,order_status: [OrderStatus.Pendding, OrderStatus.Confirmed, OrderStatus.Processing, OrderStatus.OutOfDeliver, OrderStatus.Delivered] }});

    const revenue = await dbContext.query(
      `SELECT sum(o.order_total) AS total FROM orders AS O  WHERE o.payment_status = ${PaymentStatus.paid} AND O.order_status IN (${OrderStatus.Pendding}, ${OrderStatus.Confirmed}, ${OrderStatus.Processing}, ${OrderStatus.OutOfDeliver}, ${OrderStatus.Delivered})`,
...
(SELECT product_images.image_path 
 FROM product_images WHERE product_images.id_product = OD.product_id 
 AND product_images.image_type = ${PRODUCT_IMAGE_TYPE.Feature} LIMIT 1) 
...
AND products.is_active = '1' AND products.is_deleted = '${DeletedStatus.No}' 
AND orders.order_status IN (${OrderStatus.Pendding},${OrderStatus.Confirmed},${OrderStatus.Processing},${OrderStatus.OutOfDeliver},${OrderStatus.Delivered}) ANd ORDERS.payment_status = ${PaymentStatus.paid}
GROUP BY OD.product_id, products.name, products.sku, products.slug ORDER BY count(OD.product_id) DESC LIMIT 10`,
      { type: QueryTypes.SELECT }
    );

    const items = await dbContext.query(
      `SELECT sum(order_details.quantity) AS item FROM order_details left join orders on orders.id = order_details.order_id WHERE orders.payment_status = ${PaymentStatus.paid} AND orders.order_status IN (${OrderStatus.Pendding}, ${OrderStatus.Confirmed}, ${OrderStatus.Processing}, ${OrderStatus.OutOfDeliver}, ${OrderStatus.Delivered})`,
      { type: QueryTypes.SELECT }
    );
