        ...
        include: [
          {
            ...
            attributes: [
              ...
              [
                Sequelize.literal(
                  `CASE WHEN CAST (order_details_json ->> 'product_type' AS integer) = ${AllProductTypes.Product} THEN (SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM product_images WHERE id = CAST (order_details_json ->> 'image_id' AS integer) LIMIT 1) ELSE (SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM images WHERE id = CAST (order_details_json ->> 'image_id' AS integer)) END`
                ),
                "product_image",
              ],
              [
                Sequelize.literal(
                  `CASE WHEN CAST (order_details_json ->> 'product_type' AS integer) = ${AllProductTypes.Product} THEN (SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM product_images WHERE id = CAST (order_details_json ->> 'image_id' AS integer)) WHEN  CAST (order_details_json ->> 'product_type' AS integer) = ${AllProductTypes.GiftSet_product} THEN (SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM gift_set_product_images WHERE id_product = "product_id" AND image_type = 1 AND is_deleted = '0') ELSE (SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM images where id = CAST (order_details_json ->> 'image_id' AS integer)) END`
                ),
                "product_image",
              ],
  ...
  try {
    ...
    const result = await Invoives.findOne({
      ...
      attributes: [
        ...
        "invoice_number",
        "invoice_date",
        [Sequelize.literal(`CEIL(invoice_amount)`), "invoice_amount"],
        "billing_address",
        "shipping_address",
        "order_id",
        [
          ...
          include: [
            {
              ...
              attributes: [
                ...
                "metal_rate",
                "product_details_json",
                [Sequelize.literal(`CEIL("order_invoice->order"."sub_total")`), "sub_total"],
  ...
  const paymentInfo = await axios
    ...
    .catch(async (error: any) => {
      ...
      try {
        ...
        await GiftSetProductOrderTransaction.create(order_transactions, {
          transaction: trn,
        });
        ...
        include: [
          {
            ...
            attributes: [
              ...
              [
                Sequelize.literal(
                  `(SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM gift_set_product_images WHERE id_product = "product_id" AND image_type = ${GIFT_PRODUCT_IMAGE_TYPE.Thumb} )`
                ),
                "product_image",
              ],
              [
...
export const configProductPaymentTransaction = async (req: Request) => {
  ...
  const configProductResult: any = await Invoives.findOne({
    where: { order_id: order_id },
    attributes: [
      "id",
      "invoice_number",
  ...
  const mailNewOrderPayload = {
    ...
    attachments: {
      ...
      filename: "invoice.pdf",
      content: "../../../templates/mail-template/New-Tax-invoice.html",
    },
  };
...
export const PaymentTransactionWithPaypal = async (req: Request) => {
  ...
  if (status == "SUCCESS") {
    try {
      ...
      invoiceDetails = await Invoives.create(invoiceData, { transaction: trn });
      await trn.commit();
      // return resSuccess()
    } catch (error) {
      await trn.rollback();
        ...
        include: [
          {
            ...
            attributes: [
              ...
              [
                Sequelize.literal(
                  `(SELECT CONCAT('${IMAGE_PATH}/' ,image_path) FROM product_images WHERE id_product = "product_id" AND image_type = ${PRODUCT_IMAGE_TYPE.Feature} AND id_metal_tone = CAST (order_details_json ->> 'metal_tone' AS integer) LIMIT 1)`
                ),
                "product_image",
              ],
              [
