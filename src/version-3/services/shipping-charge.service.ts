...
export const addSippingCharge = async (req: Request) => {
  ...
  try {
    ...
    const response = await checkAmountConflict(min_amount, max_amount);
...
export const updatedShippingCharge = async (req: Request) => {
  ...
  try {
    ...
    const response = await checkAmountConflict(min_amount, max_amount, id);
    if (response.code !== DEFAULT_STATUS_CODE_SUCCESS) {
      return response;
    }
    ...
    console.error(e);
    throw e; // Consider returning a user-friendly error message
  }
};
