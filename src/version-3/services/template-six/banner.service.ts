     ...
     template_six.id_category, template_six.id_collection, template_six.id_style, template_six.diamond_shape_type,
      template_six.section_type, CASE WHEN hash_tag IS NULL THEN '{}' ELSE string_to_array(hash_tag, '|') END AS hash_tag,
       image.image_path AS image_path, hover_image.image_path AS hover_image_path, title_image.image_path AS title_image_path,
        ...
        AS diamond_shape_slug, product.slug AS product_slug, product.sku AS product_sku, product.name AS product_name,
		product.pmo,product.pdo,product.product_images,product.product_categories 
        ...
        LEFT OUTER JOIN product_list_view AS product ON template_six.id_product = product.id 
        WHERE template_six.is_active = '1' AND template_six.is_deleted = '0' ORDER BY template_six.sort_order ASC)`,
     ...
     template_six.id_category, template_six.id_collection, template_six.id_style, template_six.diamond_shape_type,
      template_six.section_type, CASE WHEN hash_tag IS NULL THEN '{}' ELSE string_to_array(hash_tag, '|') END AS hash_tag,
       image.image_path AS image_path, hover_image.image_path AS hover_image_path, title_image.image_path AS title_image_path,
        ...
        LEFT OUTER JOIN diamond_shapes AS diamond_shape ON template_six.id_diamond_shape = diamond_shape.id 
        LEFT OUTER JOIN product_list_view AS product ON template_six.id_product = product.id 
        WHERE template_six.is_active = '1' AND template_six.is_deleted = '0' AND template_six.id = ${req.params.id} ORDER BY template_six.sort_order ASC)`,
      { type: QueryTypes.SELECT }
    );
