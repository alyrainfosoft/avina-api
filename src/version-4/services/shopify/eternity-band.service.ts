...
export const eternityBandConfiguratorProduct = async (req: Request) => {
    try {
        ...
        } else if (SHOPIFY_STORE_KEY.ZEGHANI_APP === req.body.app_name) {
        ...
        const productFind = await searchEternityBandProduct(
            shop,
        ...
        if (productFind.code == DEFAULT_STATUS_CODE_SUCCESS) {
            if (productFind.data.data.collection.products.edges.length <= 0) {
                const data: any = await addEternityProductInShopify(apiKey, pass, shop, req);
...
const searchEternityBandProduct = async (
  shop: any,
      ...
      filters: [
        {
          productMetafield: {
            namespace: "tcc"
            key: "band_style_plu_no"
            value: "${
              req.body.metal_tone &&
              req.body.metal_tone != undefined &&
              req.body.metal_tone != null &&
              req.body.metal_tone != ''
                ? `${req.body.sku}-${req.body.metal_tone}`
                : req.body.sku
            }"
          }
        }
...
const addEternityProductInShopify = async (apiKey: any, pass: any, shop: any, req: any) => {
  try {
    ...
    if (req.file) {
      const requestBody = JSON.stringify({
        product: {
          ...
          ${
            req.body.alt_stone && req.body.alt_stone != undefined && req.body.alt_stone != ''
              ? `<li><b>Alternate Stone: </b> ${req.body.alt_stone}</li>`
              : ''
          }
          ${
            req.body.dia_color && req.body.dia_color != undefined && req.body.dia_color != ''
              ? `<li><b>Diamond color: </b> ${req.body.dia_color}</li>`
              : ''
          }
          ${
            req.body.dia_clarity && req.body.dia_clarity != undefined && req.body.dia_clarity != ''
              ? `<li><b>Diamond Clarity: </b> ${req.body.dia_clarity}</li>`
              : ''
          }
          ${
            req.body.dia_cut && req.body.dia_cut != undefined && req.body.dia_cut != ''
              ? `<li><b>Diamond Cut: </b> ${req.body.dia_cut}</li>`
              : ''
          }
          ${
            req.body.dia_size && req.body.dia_size != undefined && req.body.dia_size != ''
              ? `<li><b>Diamond Size: </b> ${req.body.dia_size}</li>`
              : ''
          }
          ${
            req.body.dia_type && req.body.dia_type != undefined && req.body.dia_type != ''
              ? `<li><b>Diamond Type: </b> ${req.body.dia_type}</li>`
              : ''
          }
          </ul>
          `,
