...
export const addShopBySection = async (req: Request) => {
  try {
    ...
    try {
      ...
      const TemplateThreeShopBy =await TemplateThreeData.create(
        {
        ...
        await addActivityLogs(req,req?.body?.session_res?.client_id,[{
          old_data: null,
          new_data: {
            template_three_shopby_id: TemplateThreeShopBy?.dataValues?.id, data: {
              ...TemplateThreeShopBy?.dataValues
            }}
        }], TemplateThreeShopBy?.dataValues?.id, LogsActivityType.Add, LogsType.TemplateThreeShopBy, req?.body?.session_res?.id_app_user,trn)
...
export const updateShopBySection = async (req: Request) => {
  try {
    ...
    try {
      ...
      const AfterUpdatefindShopBySection = await TemplateThreeData.findOne({
        where: { id: req.params.id, is_deleted: DeletedStatus.No  },transaction:trn
      });

      await addActivityLogs(req,req?.body?.session_res?.client_id,[{
        old_data: { template_three_shopby_id: findShopBySection?.dataValues?.id, data: {...findShopBySection?.dataValues} },
        new_data: {
          template_three_shopby_id: AfterUpdatefindShopBySection?.dataValues?.id, data: { ...AfterUpdatefindShopBySection?.dataValues }
        }
      }], findShopBySection?.dataValues?.id, LogsActivityType.Edit, LogsType.TemplateThreeShopBy, req?.body?.session_res?.id_app_user,trn)
...
export const deleteShopBySection = async (req: Request) => {
  try {
    ...
    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      old_data: { template_three_shopby_id: findShopBySection?.dataValues?.id, data:{...findShopBySection?.dataValues} },
      new_data: {
        template_three_shopby_id: findShopBySection?.dataValues?.id, data: {
          ...findShopBySection?.dataValues, is_deleted: DeletedStatus.yes,
          modified_by: req?.body?.session_res?.id_app_user,
          modified_date: getLocalDate(),
        }
      }
    }], findShopBySection?.dataValues?.id, LogsActivityType.Delete, LogsType.TemplateThreeShopBy, req?.body?.session_res?.id_app_user)
...
export const getShopBySection = async (req: Request) => {
  try {
    const { TemplateThreeData,Image, Collection,SettingTypeData,CategoryData} = initModels(req);
    let paginationProps = {};
...
export const statusUpdateForShopBySection = async (req: Request) => {
  try {
    ...
    }], findShopBySection?.dataValues?.id, LogsActivityType.StatusUpdate, LogsType.TemplateThreeShopBy, req?.body?.session_res?.id_app_user)
...
export const templateThreeAllSectionListForUser = async (req: Request) => {
  try {
    const { TemplateThreeData,Image, SettingTypeData,CategoryData, DiamondShape,Collection  } = initModels(req);
    ...
    const result = await TemplateThreeData.findAll({
      ...
      include: [
        { model: Image, as: "image", attributes: [],where:{company_info_id:company_info_id?.data} },
        { model: Image, as: "hover_image", attributes: [],where:{company_info_id:company_info_id?.data} },
        { model: CategoryData, as: "category", attributes: [],where:{company_info_id:company_info_id?.data} },
        { model: SettingTypeData, as: "style", attributes: [],where:{company_info_id:company_info_id?.data} },
        { model: Collection, as: "collection", attributes: [],where:{company_info_id:company_info_id?.data} },
        { model: DiamondShape, as: "diamond_shape", attributes: [],where:{company_info_id:company_info_id?.data} },
      ],
    });
...
export const templateThreeAllSectionDetailForUser = async (req: Request) => {
  try {
    const { TemplateThreeData, Image, SettingTypeData, CategoryData, Collection, DiamondShape } = initModels(req);
    const company_info_id = await getCompanyIdBasedOnTheCompanyKey(req?.query,req.body.db_connection);
    if(company_info_id.code !== DEFAULT_STATUS_CODE_SUCCESS){
      return company_info_id;
    }
    const result = await TemplateThreeData.findOne({
      ...
      include: [
        { model: Image, as: "image", attributes: [],where:{company_info_id :req?.body?.session_res?.client_id} },
        { model: Image, as: "hover_image", attributes: [],where:{company_info_id :req?.body?.session_res?.client_id} },
        { model: CategoryData, as: "category", attributes: [],where:{company_info_id :req?.body?.session_res?.client_id} },
        { model: SettingTypeData, as: "style", attributes: [],where:{company_info_id :req?.body?.session_res?.client_id} },
        { model: Collection, as: "collection", attributes: [],where:{company_info_id :req?.body?.session_res?.client_id} },
        { model: DiamondShape, as: "diamond_shape", attributes: [],where:{company_info_id :req?.body?.session_res?.client_id} },
      ],
    });
