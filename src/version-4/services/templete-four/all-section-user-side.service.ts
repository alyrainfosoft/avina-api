...
import { ActiveStatus, DeletedStatus, PRICE_CORRECTION_PRODUCT_TYPE, SingleProductType } from "../../../utils/app-enumeration";
export const templateFoursAllSectionListForUser = async (req: any) => {
    try {
    ...
    if(company_info_id.code !== DEFAULT_STATUS_CODE_SUCCESS){
      ...
      const result = await (req.body.db_connection).query(
        `(SELECT 
            template_four.id,
            template_four.title,
            template_four.sub_title,
            template_four.description,
            template_four.sub_description,
            template_four.title_color,
            template_four.sub_title_color,
            template_four.description_color,
            template_four.sub_description_color,
            template_four.id_categories,
            category.category_name AS category_name,
            category.slug AS slug,
            template_four.link,
            template_four.button_name,
            template_four.button_color,
            template_four.button_text_color,
            template_four.is_button_transparent,
            template_four.button_hover_color,
            template_four.button_text_hover_color,
            template_four.section_type,
            template_four.bg_color,
            id_title_image,
            ...
            sort_order,
          CASE
            WHEN "template_four"."id_products" IS NULL or template_four.section_type != 'latest_collection' OR "template_four"."id_products" = '' THEN '{}'::int[]
            ELSE string_to_array("template_four"."id_products", '|')::int[]
          END as latest_collection,
          template_four.product_ids AS products
          FROM template_four
          LEFT OUTER JOIN images AS title_image ON template_four.id_title_image = title_image.id
          LEFT OUTER JOIN images AS bg_image ON template_four.id_bg_image = bg_image.id
          LEFT OUTER JOIN categories AS category ON template_four.id_categories = category.id
          WHERE template_four.is_active = '${ActiveStatus.Active}' AND template_four.is_deleted ='${DeletedStatus.No}' AND template_four.company_info_id = ${company_info_id?.data}
          ORDER BY template_four.sort_order ASC
          )`,
      ...
      let addCategoryProductDetail = [];
        if(templateData.latest_collection && templateData.latest_collection.length > 0) {
 ...
 SELECT products.id,
  ...
  GROUP BY products.id
          `, { type: QueryTypes.SELECT,
            replacements: {
              product_ids: templateData.latest_collection
            }
        });
          addCategoryProductDetail = product;
        }
        let addProductDetail = [];
        if(templateData.products && templateData.products != null && templateData.products.length > 0 && templateData.section_type == 'new_collection') {
        const product =  await req.body.db_connection.query(`
            WITH filtered_pmo AS (
         SELECT DISTINCT ON (pmo.id_product) pmo.id,
            pmo.id_product,
 ...
 SELECT products.id,
  ...
  GROUP BY products.id
       ...
       resultWithProduct.push({...templateData, products: addProductDetail, latest_collection: addCategoryProductDetail})
      }
    ...
    const chooseSettingProductRoundingValue = findRoundingValue.find((item: any) => item.product_type === PRICE_CORRECTION_PRODUCT_TYPE.ChooseSettingProduct);
      ...
      for (const item of resultWithProduct) { 
        ...
        if(item.latest_collection && data.latest_collection.length > 0) {
          data.latest_collection = await Promise.all(item.latest_collection.map(async(t: any) => {
          ...
          const chooseSettingProductRoundValue = data.product_type == SingleProductType.DynemicPrice || SingleProductType.cataLogueProduct ? chooseSettingProductRoundingValue && chooseSettingProductRoundingValue.round_off ? {value: chooseSettingProductRoundingValue.round_off, flag: true} : {value: 0, flag: false} : {value: 0, flag: false}
            return {
              ...
              pmo: await Promise.all(t.pmo.map(async(value: any) => ({
                ...
                compare_price: await req.formatPrice(value.compare_price,chooseSettingProductType,),
              }))),
            }
          } ))          
        }
    ...
    try {
      const result = await (req.body.db_connection).query(
        `(SELECT 
          ...
          END as latest_collection,
          CASE
            WHEN template_four.product_ids IS NULL THEN '{}'::json
            ELSE template_four.product_ids AS products
            END AS products
          FROM template_four
          LEFT OUTER JOIN images AS title_image ON template_four.id_title_image = title_image.id
          LEFT OUTER JOIN images AS bg_image ON template_four.id_bg_image = bg_image.id
          LEFT OUTER JOIN categories AS category ON template_four.id_categories = category.id
          WHERE template_four.is_active = '${ActiveStatus.Active}' AND template_four.is_deleted ='${DeletedStatus.No}' AND template_four.id = ${req.params.id} ORDER BY template_four.sort_order ASC)`,
        { type: QueryTypes.SELECT }
      );
