...
export const addBannerSection = async (req: Request) => {
  try {
    const {
      title,
      sub_title,
    ...
    try {
      ...
      const TemplateFourOfferSlider = await TemplateFourData.create(
        {
          section_type: TemplateFour.Banner,
      ...
      }], TemplateFourOfferSlider?.dataValues?.id, LogsActivityType.Add, LogsType.templateFourBanner, req?.body?.session_res?.id_app_user,trn)
...
export const updateBannerSection = async (req: Request) => {
  try {
    ...
    try {
      ...
      }], findDiamondShapeSection?.dataValues?.id, LogsActivityType.Edit, LogsType.templateFourBanner, req?.body?.session_res?.id_app_user,trn)
...
export const deleteBannerSection = async (req: Request) => {
  try {
    const { TemplateFourData } = initModels(req);
    const BannerSection = await TemplateFourData.findOne({
      where: { id: req.params.id, is_deleted: DeletedStatus.No,company_info_id :req?.body?.session_res?.client_id },
    });

    if (!(BannerSection && BannerSection.dataValues)) {
      return resNotFound({ message: NOT_FOUND_MESSAGE });
    }
    ...
    await TemplateFourData.update(
      ...
      { where: { id: BannerSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    );

    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      old_data: { temaplate_seven_offers_slider_id: BannerSection?.dataValues?.id, data: {...BannerSection?.dataValues} },
      new_data: {
        temaplate_seven_offers_slider_id: BannerSection?.dataValues?.id, data: {
          ...BannerSection?.dataValues, is_deleted: DeletedStatus.yes,
          modified_by: req?.body?.session_res?.id_app_user,
          modified_date: getLocalDate(),
        },BannerSection
      }
    }], BannerSection?.dataValues?.id, LogsActivityType.Delete, LogsType.templateFourBanner, req?.body?.session_res?.id_app_user)
...
export const getBannerSection = async (req: Request) => {
  try {
    const { TemplateFourData,Image } = initModels(req);
    let paginationProps = {};
...
export const statusUpdateForBannerSection = async (req: Request) => {
  try {
    const { TemplateFourData } = initModels(req);
    const BannerSection = await TemplateFourData.findOne({
      where: {
        id: req.params.id,
        section_type: { [Op.eq]: TemplateFour.Banner },
        is_deleted: DeletedStatus.No,
        company_info_id :req?.body?.session_res?.client_id
      },
    });
    ...
    await TemplateFourData.update(
      {
        is_active: statusUpdateValue(BannerSection),
        modified_date: getLocalDate(),
        modified_by: req.body.session_res.id_app_user,
      },
      { where: { id: BannerSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    ...
    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      ...
      new_data: {
        temaplate_seven_offers_slider_id: BannerSection?.dataValues?.id, data: {
          ...BannerSection?.dataValues, is_active: statusUpdateValue(BannerSection),
          modified_date: getLocalDate(),
          modified_by: req?.body?.session_res?.id_app_user,
        }
      }
    }], BannerSection?.dataValues?.id, LogsActivityType.StatusUpdate, LogsType.templateFourBanner, req?.body?.session_res?.id_app_user)
