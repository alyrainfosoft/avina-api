...
export const addUpdateTemplateFourProducts = async (req: Request) => {
    try {
        const { 
        title,
        sub_title, 
        ...
        section_type = 'new_collection' } = req.body
        const { TemplateFourData } = initModels(req);
        const findSection = await TemplateFourData.findOne({ where: { section_type: { [Op.eq]: section_type }, is_deleted: DeletedStatus.No } })
        ...
        try {
            ...
            }, { where: { section_type: { [Op.eq]: section_type }, is_deleted: DeletedStatus.No } })
           

            const afterUpdateFindSection = await TemplateFourData.findOne({ where: { section_type: { [Op.eq]: section_type }, is_deleted: DeletedStatus.No } })
            ...
            }], findSection?.dataValues?.id, LogsActivityType.Edit, LogsType.templateFourNewCollection, req?.body?.session_res?.id_app_user,trn)

        }else{
        
            const data = await TemplateFourData.create({
                title:title,
                sub_title:sub_title, 
                link:link,
                button_name: button_name,
            ...
            }], data.dataValues.id, LogsActivityType.Add, LogsType.templateFourNewCollection, req.body.session_res.id_app_user,trn)
        }
            await trn.commit()
...
export const getALlTemplateFourProducts = async (req: Request) => {
    try {
        const { section_type = "new_collection" } = req.params
        const { TemplateFourData } = initModels(req);
        const data = await TemplateFourData.findOne(
            {
                where: { is_deleted: DeletedStatus.No, section_type: section_type,company_info_id :req?.body?.session_res?.client_id },
