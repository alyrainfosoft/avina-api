...
export const addLominousDesignSection = async (req: Request) => {
  try {
    ...
    try {
      ...
      const TemplateSevenLominousDesignSection = await TemplateSevenData.create(
        {
          section_type: TemplateSevenSectionType.LuminousDesignSection, 
      ...
      await addActivityLogs(req,req?.body?.session_res?.client_id,[{
        old_data: null,
        new_data: {
          temaplate_seven_luminous_design_id: TemplateSevenLominousDesignSection?.dataValues?.id, data: {
            ...TemplateSevenLominousDesignSection?.dataValues
          }
        }
      }], TemplateSevenLominousDesignSection?.dataValues?.id, LogsActivityType.Add, LogsType.TemplateSevenLuminousDesignSection, req?.body?.session_res?.id_app_user,trn)
...
export const updateLominousDesignSection = async (req: Request) => {
  try {
    ...
    try {
      ...
      }], findDiamondShapeSection?.dataValues?.id, LogsActivityType.Edit, LogsType.TemplateSevenLuminousDesignSection, req?.body?.session_res?.id_app_user,trn)
...
export const deleteLominousDesignSection = async (req: Request) => {
  try {
    const {TemplateSevenData} = initModels(req);
    const LominousDesignSection = await TemplateSevenData.findOne({
      where: { id: req.params.id, is_deleted: DeletedStatus.No,company_info_id :req?.body?.session_res?.client_id },
    });

    if (!(LominousDesignSection && LominousDesignSection.dataValues)) {
      return resNotFound({ message: NOT_FOUND_MESSAGE });
    }
    ...
    await TemplateSevenData.update(
      ...
      { where: { id: LominousDesignSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    );

    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      old_data: { temaplate_seven_luminous_design_id: LominousDesignSection?.dataValues?.id, data: {...LominousDesignSection?.dataValues}},
      new_data: {
        temaplate_seven_luminous_design_id: LominousDesignSection?.dataValues?.id, data: {
          ...LominousDesignSection?.dataValues, is_deleted: DeletedStatus.yes,
          modified_by: req?.body?.session_res?.id_app_user,
          modified_date: getLocalDate(),
        }
      }
    }], LominousDesignSection?.dataValues?.id, LogsActivityType.Delete, LogsType.TemplateSevenLuminousDesignSection, req?.body?.session_res?.id_app_user)
...
export const getLominousDesignSection = async (req: Request) => {
  try {
...
export const statusUpdateForLominousDesignSection = async (req: Request) => {
  try {
    const {TemplateSevenData} = initModels(req);
    const LominousDesignSection = await TemplateSevenData.findOne({
      where: {
        id: req.params.id,
        section_type: { [Op.eq]: TemplateSevenSectionType.LuminousDesignSection },
        is_deleted: DeletedStatus.No,
        company_info_id :req?.body?.session_res?.client_id,
      },
    });
    ...
    await TemplateSevenData.update(
      {
        is_active: statusUpdateValue(LominousDesignSection),
        modified_date: getLocalDate(),
        modified_by: req.body.session_res.id_app_user,
      },
      { where: { id: LominousDesignSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    ...
    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      ...
      new_data: {
        temaplate_seven_luminous_design_id: LominousDesignSection?.dataValues?.id, data: {
          ...LominousDesignSection?.dataValues, is_active: statusUpdateValue(LominousDesignSection),
          modified_date: getLocalDate(),
          modified_by: req?.body?.session_res?.id_app_user,
        }
      }
    }], LominousDesignSection?.dataValues?.id, LogsActivityType.StatusUpdate, LogsType.TemplateSevenLuminousDesignSection, req?.body?.session_res?.id_app_user)
