...
export const addOffersSliderSection = async (req: Request) => {
  try {
    const {
      title,
      sub_title,
    ...
    try {
      ...
      const TemplateSevenOfferSlider = await TemplateSevenData.create(
        {
          section_type: TemplateSevenSectionType.OffersSliderSection,
          link: link,
      ...
      await addActivityLogs(req,req?.body?.session_res?.client_id,[{
        old_data: null,
        new_data: {
          temaplate_seven_offers_slider_id: TemplateSevenOfferSlider?.dataValues?.id, data: {
            ...TemplateSevenOfferSlider?.dataValues
          },section_type: TemplateSevenSectionType.OffersSliderSection,
        }
      }], TemplateSevenOfferSlider?.dataValues?.id, LogsActivityType.Add, LogsType.TemplateSevenOffersSliderSection, req?.body?.session_res?.id_app_user,trn)
...
export const updateOffersSliderSection = async (req: Request) => {
  try {
    ...
    try {
      ...
      }], findDiamondShapeSection?.dataValues?.id, LogsActivityType.Edit, LogsType.TemplateSevenOffersSliderSection, req?.body?.session_res?.id_app_user,trn)
...
export const deleteOffersSliderSection = async (req: Request) => {
  try {
    const { TemplateSevenData } = initModels(req);
    const OffersSliderSection = await TemplateSevenData.findOne({
      where: { id: req.params.id, is_deleted: DeletedStatus.No,company_info_id :req?.body?.session_res?.client_id },
    });

    if (!(OffersSliderSection && OffersSliderSection.dataValues)) {
      return resNotFound({ message: NOT_FOUND_MESSAGE });
    }
    ...
    await TemplateSevenData.update(
      ...
      { where: { id: OffersSliderSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    );

    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      old_data: { temaplate_seven_offers_slider_id: OffersSliderSection?.dataValues?.id, data: {...OffersSliderSection?.dataValues} },
      new_data: {
        temaplate_seven_offers_slider_id: OffersSliderSection?.dataValues?.id, data: {
          ...OffersSliderSection?.dataValues, is_deleted: DeletedStatus.yes,
          modified_by: req?.body?.session_res?.id_app_user,
          modified_date: getLocalDate(),
        },OffersSliderSection
      }
    }], OffersSliderSection?.dataValues?.id, LogsActivityType.Delete, LogsType.TemplateSevenOffersSliderSection, req?.body?.session_res?.id_app_user)
...
export const getOffersSliderSection = async (req: Request) => {
  try {
    const { TemplateSevenData,Image } = initModels(req);
    let paginationProps = {};
...
export const statusUpdateForOffersSliderSection = async (req: Request) => {
  try {
    const { TemplateSevenData } = initModels(req);
    const OffersSliderSection = await TemplateSevenData.findOne({
      where: {
        id: req.params.id,
        section_type: { [Op.eq]: TemplateSevenSectionType.OffersSliderSection },
        is_deleted: DeletedStatus.No,
        company_info_id :req?.body?.session_res?.client_id
      },
    });
    ...
    await TemplateSevenData.update(
      {
        is_active: statusUpdateValue(OffersSliderSection),
        modified_date: getLocalDate(),
        modified_by: req.body.session_res.id_app_user,
      },
      { where: { id: OffersSliderSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    ...
    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      ...
      new_data: {
        temaplate_seven_offers_slider_id: OffersSliderSection?.dataValues?.id, data: {
          ...OffersSliderSection?.dataValues, is_active: statusUpdateValue(OffersSliderSection),
          modified_date: getLocalDate(),
          modified_by: req?.body?.session_res?.id_app_user,
        }
      }
    }], OffersSliderSection?.dataValues?.id, LogsActivityType.StatusUpdate, LogsType.TemplateSevenOffersSliderSection, req?.body?.session_res?.id_app_user)
