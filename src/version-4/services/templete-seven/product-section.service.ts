...
export const addUpdateTemplateSevenProducts = async (req: Request) => {
    try {
        const { products = [], section_type = 'best_seller' } = req.body
        const { TemplateSevenData } = initModels(req);
        const findSection = await TemplateSevenData.findOne({ where: { section_type: { [Op.eq]: section_type }, is_deleted: DeletedStatus.No } })
        ...
        try {
            ...
            const afterUpdateFindSection = await TemplateSevenData.findOne({ where: { section_type: { [Op.eq]: section_type }, is_deleted: DeletedStatus.No } })
            ...
            }], data.dataValues.id, LogsActivityType.Add, LogsType.TemplateSevenproductsSection, req.body.session_res.id_app_user,trn)
        }
            await trn.commit()
