...
export const addStunningJewelrySection = async (req: Request) => {
  try {
    ...
    try {
      ...
      const TemplateSevenStunningJewelry = await TemplateSevenData.create(
        {
      ...
      await addActivityLogs(req,req?.body?.session_res?.client_id,[{
        old_data: null,
        new_data: {
          temaplate_seven_stunning_jewellry_id: TemplateSevenStunningJewelry?.dataValues?.id, data: {
            ...TemplateSevenStunningJewelry?.dataValues
          }
        }
      }], TemplateSevenStunningJewelry?.dataValues?.id, LogsActivityType.Add, LogsType.TemplateSevenStunningJewelsSection, req?.body?.session_res?.id_app_user,trn)
    ...
    try {
      ...
      if (stunningJewelrySection.dataValues.id_offer_image) {
        findOfferImage = await Image.findOne({
          where: { id: stunningJewelrySection.dataValues.id_offer_image,company_info_id :req?.body?.session_res?.client_id },
          transaction: trn,
        });
      }
      ...
      const AfterUpdatestunningJewelrySection = await TemplateSevenData.findOne({
      ...
      }], stunningJewelrySection?.dataValues?.id, LogsActivityType.Edit, LogsType.TemplateSevenStunningJewelsSection, req?.body?.session_res?.id_app_user,trn)
...
export const deleteStunningJewelrySection = async (req: Request) => {
  try {
    ...
    await TemplateSevenData.update(
      ...
      { where: { id: StunningJewelrySection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    );

    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      old_data: { temaplate_seven_stunning_jewellry_id: StunningJewelrySection?.dataValues?.id, data: {...StunningJewelrySection?.dataValues}},
      new_data: {
        temaplate_seven_stunning_jewellry_id: StunningJewelrySection?.dataValues?.id, data: {
          ...StunningJewelrySection?.dataValues, is_deleted: DeletedStatus.yes,
          modified_by: req?.body?.session_res?.id_app_user,
          modified_date: getLocalDate(),
        }
      }
    }], StunningJewelrySection?.dataValues?.id, LogsActivityType.Delete, LogsType.TemplateSevenStunningJewelsSection, req?.body?.session_res?.id_app_user)
...
export const statusUpdateForStunningJewelrySection = async (req: Request) => {
  try {
    ...
    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      ...
      new_data: {
        temaplate_seven_stunning_jewellry_id: StunningJewelrySection?.dataValues?.id, data: {
          ...StunningJewelrySection?.dataValues, is_active: statusUpdateValue(StunningJewelrySection),
          modified_date: getLocalDate(),
          modified_by: req?.body?.session_res?.id_app_user,
        }
      }
    }], StunningJewelrySection?.dataValues?.id, LogsActivityType.StatusUpdate, LogsType.TemplateSevenStunningJewelsSection, req?.body?.session_res?.id_app_user)
