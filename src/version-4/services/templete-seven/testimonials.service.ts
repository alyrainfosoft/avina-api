...
export const addAndUpdatepdateTestimonialsSection = async (req: Request) => {
  try {
    ...
    const findDiamondShapeSection = await TemplateSevenData.findOne({
      where: {section_type: TemplateSevenSectionType.TestimonialSection, is_deleted: DeletedStatus.No,company_info_id :req?.body?.session_res?.client_id },
    });
    ...
    try {
      ...
      }], findDiamondShapeSection?.dataValues?.id, LogsActivityType.Edit, LogsType.TemplateSevenTestimonialSection, req?.body?.session_res?.id_app_user,trn)
      ...
      const TemplateSevenTestimonial = await TemplateSevenData.create(
        {
          section_type: TemplateSevenSectionType.TestimonialSection,
          link: link,
      ...
      await addActivityLogs(req,req?.body?.session_res?.client_id,[{
        old_data: null,
        new_data: {
          temaplate_seven_testimonials_id: TemplateSevenTestimonial?.dataValues?.id, data: {
            ...TemplateSevenTestimonial?.dataValues
          }
        }
      }], TemplateSevenTestimonial?.dataValues?.id, LogsActivityType.Add, LogsType.TemplateSevenTestimonialSection, req?.body?.session_res?.id_app_user,trn)
    }
      await trn.commit();
...
export const deleteTestimoninalsSection = async (req: Request) => {
  try {
    ...
    await TemplateSevenData.update(
      ...
      { where: { id: TestimonialsSection.dataValues.id,company_info_id :req?.body?.session_res?.client_id } }
    );

    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      old_data: { temaplate_seven_testimonials_id: TestimonialsSection?.dataValues?.id, data: {...TestimonialsSection?.dataValues}},
      new_data: {
        temaplate_seven_testimonials_id: TestimonialsSection?.dataValues?.id, data: {
          ...TestimonialsSection?.dataValues, is_deleted: DeletedStatus.yes,
          modified_by: req?.body?.session_res?.id_app_user,
          modified_date: getLocalDate(),
        }
      }
    }], TestimonialsSection?.dataValues?.id, LogsActivityType.Delete, LogsType.TemplateSevenTestimonialSection, req?.body?.session_res?.id_app_user)
...
export const statusUpdateForTestimonialsSection = async (req: Request) => {
  try {
    ...
    await addActivityLogs(req,req?.body?.session_res?.client_id,[{
      ...
      new_data: {
        temaplate_seven_testimonials_id: TestimonialsSection?.dataValues?.id, data: {
          ...TestimonialsSection?.dataValues, is_active: statusUpdateValue(TestimonialsSection),
          modified_date: getLocalDate(),
          modified_by: req?.body?.session_res?.id_app_user,
        }
      }
    }], TestimonialsSection?.dataValues?.id, LogsActivityType.StatusUpdate, LogsType.TemplateSevenTestimonialSection, req?.body?.session_res?.id_app_user)
    return resSuccess({ message: RECORD_UPDATE_SUCCESSFULLY });
  } catch (error) {
    throw error;
  }
