...
const addProduct = async (requestBody: any, req: Request) => {
  const {ConfigProduct,DiamondGroupMaster,ConfigProductMetals,ConfigProductDiamonds,DiamondShape,DiamondCaratSize, StoneData, Colors,ClarityData} = initModels(req)
  const {
    sku,
    price,
    product_title,
  ...
  const product = await ConfigProduct.findOne({
    where: { sku: { [Op.iLike]: `${sku}` }, is_deleted: DeletedStatus.No },
    attributes: [
      "id",
      "head_no",
      "shank_no",
